...
/* Builds one weighted entry per community report plus one for the ref's
   ...
   everyone else.
   reports: [{ accountId, reporterTrust, homeScore, awayScore, homeScorers, awayScorers, motm, yellowCards, redCards }]
...
function buildEntries(match, reports, refTrust) {
  const entries = reports.map((r) => ({
    ...
    weight: trustWeight(r.reporterTrust ?? 0),
    isOfficial: false,
...
function getMatchConsensus(match, refTrustByName) {
  ...
  const reports = (match.reports || []).map((r) => ({ ...r, reporterTrust: r.account?.trust ?? 0 }));
